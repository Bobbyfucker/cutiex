...
from YinsRobot.modules.helper_funcs.admin_rights import (
    user_can_changeinfo,
    user_can_promote,
)
...
def setchatpic(update: Update, context: CallbackContext):
    ...
    if msg.reply_to_message:
        if msg.reply_to_message.photo:
            ...
            msg.reply_text(
                "Anda hanya dapat mengatur beberapa foto sebagai gambar obrolan!"
            )
            return
        ...
        msg.reply_text(
            "Balas ke beberapa foto atau file untuk mengatur gambar obrolan baru!"
        )
...
def set_desc(update: Update, context: CallbackContext):
    ...
    if len(tesc) >= 2:
        ...
        return msg.reply_text(
            "Menyetel deskripsi kosong tidak akan menghasilkan apa-apa!"
        )
    try:
        if len(desc) > 255:
...
def setchat_title(update: Update, context: CallbackContext):
    ...
    if not title:
        msg.reply_text(
            "Masukkan beberapa teks untuk menetapkan judul baru di obrolan Anda!"
        )
        return
...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    if user_member.status in ("administrator", "creator"):
        message.reply_text(
            "Bagaimana cara saya mempromosikan seseorang yang sudah menjadi admin?"
        )
        return

    if user_id == bot.id:
        message.reply_text(
            "Saya tidak bisa mempromosikan diri saya sendiri! Dapatkan admin untuk melakukannya untuk saya."
        )
        return
    ...
    try:
        ...
        if err.message == "User_not_mutual_contact":
            message.reply_text(
                "Saya tidak dapat mempromosikan seseorang yang tidak ada dalam grup."
            )
        else:
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    if user_member.status == "creator":
        message.reply_text(
            "Orang ini MENCIPTAKAN obrolan, bagaimana saya menurunkannya?"
        )
        return
    ...
    if user_id == bot.id:
        message.reply_text(
            "Saya tidak bisa menurunkan diri saya sendiri! Dapatkan admin untuk melakukannya untuk saya."
        )
        return
...
def unpin(update: Update, context: CallbackContext):
    ...
    if not prev_message and is_group:
        try:
            ...
            msg.reply_text("Lepas pin pesan yang terakhir disematkan.")
        except BadRequest as excp:
